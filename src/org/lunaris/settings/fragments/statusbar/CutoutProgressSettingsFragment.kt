...
import org.lunaris.settings.utils.toArgb
import org.lunaris.settings.utils.toHexString
...
class CutoutProgressSettingsFragment : SettingsPreferenceFragment(),
    ...
    companion object {
        ...
        private const val MUSIC_COLOR_MODE_ALBUM = 0
        private const val MUSIC_COLOR_MODE_CUSTOM = 1
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        val storedMusicMode = readSecureInt(KEY_MUSIC_COLOR_MODE, MUSIC_COLOR_MODE_ALBUM)
    ...
    private fun updateColorPickerVisibility(mode: Int, key: String) {
        if (key == KEY_RING_COLOR_MODE) {
            val isCustom = mode == COLOR_MODE_CUSTOM
            ringColorPref.isVisible = isCustom
        } else if (key == KEY_MUSIC_COLOR_MODE) {
            val isCustom = mode == MUSIC_COLOR_MODE_CUSTOM
            musicColorPref.isVisible = isCustom
        }
    }
